	...
	private static void borraremple(int id) throws IOException {
		...
		try {
			...
			if (posicion >= file.length())
				System.out.println("ID: " + id + ", NO EXISTE EMPLEADO (sobrepasa)...");
			else {
				file.seek(posicion);// nos posicionamos
				int idd = file.readInt(); // obtengo id de empleado
				if (idd == id) {
						System.out.println("ID: " + id + ", EXISTE, BORRADO LÓGICO. ");
						file.seek(posicion);
						//ponemos a 0 e id
						file.writeInt(0);
						String apellido="          "; // fijar a 10 carac
					...
					System.out.println("ID: " + id + ", NO EXISTE, ES HUECO. NO SE BORRA...");
